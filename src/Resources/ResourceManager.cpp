...
{
	std::size_t found = excecutablePath.find_last_of("/\\");
	m_path = excecutablePath.substr(0, found);
}
...
std::shared_ptr<Renderer::ShaderProgram> ResourceManager::loadShaders(const std::string& shaderName, const std::string& vertexPath, const std::string& fragmentPath)
{
	...
	std::shared_ptr<Renderer::ShaderProgram>& newShader =
		...
		.first->second;
	if (newShader->isCompilled())
	{
		return newShader;
	}

	std::cerr << "Can't laod shader program:\n"
		<< "Vertex: " << vertexPath << "\n"
...
std::shared_ptr<Renderer::Texture2D> ResourceManager::getTexture(const std::string& textureName)
{
	...
	std::cerr << "Can't find the texure: " << textureName << std::endl;
	return nullptr;
}